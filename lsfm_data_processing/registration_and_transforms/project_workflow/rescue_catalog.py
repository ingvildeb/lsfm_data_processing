...
def _source_variant_name(source_run: str, variant: str) -> str:
    ...
    if not source_token:
        raise ValueError("An exact retry requires a usable source-run path")
    return f"from_{source_token}_{variant}"


def expected_rescue_run_paths(
    request: RescueRequest, strategy: RescueStrategy
) -> tuple[str, ...]:
    """Return the runs generated by a canonical non-masking request."""

    definition = canonical_rescue_definition_lookup().get(strategy.strategy_id)
    if definition is None or strategy.kind is RescueStrategyKind.MASKING:
        return ()
